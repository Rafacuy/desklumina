  ...
  return results
    .map((r) => {
      ...
      const lines = [
        ...
        r.summary?.totalMatches !== undefined ? `matches=${r.summary.totalMatches}` : "",
        r.selectedFile ? `file=${r.selectedFile}` : "",
        r.files && r.files.length > 0 ? "files:" : "",
        ...(r.files ? r.files.slice(0, 3).map((file: any) => `  - ${file.path}`) : []),
        r.files && r.files.length > 3 ? `  - (...and ${r.files.length - 3} more)` : "",
        r.preview?.content ? `preview=\n${formatTruncated(r.preview.content, 200)}` : "",
