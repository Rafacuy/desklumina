...
export const LOADER_ASSET_DIR = `${Bun.env.HOME}/.config/desklumina/assets/loader`;
...
export function startLoader() {
  currentIndex = 0;
  Bun.stdout.write(t(MESSAGES[currentIndex] || ""));
  
  loaderInterval = setInterval(() => {
    currentIndex = (currentIndex + 1) % MESSAGES.length;
    Bun.stdout.write(`\r${t(MESSAGES[currentIndex] || "")}`);
  }, 1200);
}
...
export function stopLoader() {
  if (loaderInterval) {
    ...
    loaderInterval = null;
    Bun.stdout.write("\r" + " ".repeat(80) + "\r");
  }
}
