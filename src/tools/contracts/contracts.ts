...
export const TOOL_CONTRACTS: ToolContract[] = [
  ...
  {
    name: "music",
    description: "Control media playback and inspect current track status. Before calling this for any playback-affecting action (play, resume, pause, stop, next, prev), execute a track query first with {\"current\": true} unless a backend is already known from this turn. Pass the resolved backend explicitly.",
    schema:
      'music {"action"?: "play"|"resume"|"pause"|"stop"|"next"|"prev"|"volume_up"|"volume_down", "backend"?: "mpc"|"playerctl", "current"?: boolean, "backends"?: ["all"|"mpc"|"playerctl"]}',
    types: { action: "enum (optional)", backend: "enum (optional)", current: "boolean (optional)", backends: "enum[] (optional)" },
    requiredArgs: [],
    optionalArgs: ["action", "backend", "current", "backends"],
    validFormats: [
      'music {"action": "play"}',
      'music {"action": "next", "backend": "mpc"}',
      'music {"action": "pause", "backend": "playerctl"}',
      'music {"action": "volume_up"}',
      'music {"current": true}',
      'music {"current": true, "backends": ["all"]}',
      'music {"current": true, "backends": ["mpc"]}',
      'music {"current": true, "backends": ["playerct"]}',
    ],
    invalidFormats: [
      "music play",
      'music "play"',
      "music volume_up",
      'music {"action": "next", "backend": "mpd"}',
      'music {"action": "next", "backend": "spotify"}',
      'music {"current": true, "backends": ["invalid"]}',
    ],
    ...
    output: {
      success: "Status message or track list. Track query returns JSON object: { tracks: TrackInfo[], activePrimaryBackend: 'mpc'|'playerctl'|null }.",
      failure: "Error string: 'No available media players found', 'Backend not available: <name>', or malformed backend error.",
      empty: "{ tracks: [], activePrimaryBackend: null } when nothing is playing during a track query.",
    },
    failure: {
      retriable: ["Backend timeout"],
      nonRetriable: ["Invalid action", "No active players", "Unknown backend name", "Explicit backend unavailable", "Malformed backends value"],
      retryLimit: 1,
      permissionBehavior: "N/A",
      malformedInputBehavior:
        "Falls back to string mapping for action, or returns exit code 2 and error message for invalid backends value.",
    },
    formatAnchors: [
      '{"tool":"music","args":"{\\"action\\":\\"play\\",\\"backend\\":\\"mpc\\"}"}',
      '{"tool":"music","args":"{\\"current\\":true,\\"backends\\":[\\"all\\"]}"}',
    ],
  },
  {
    name: "clipboard",
  ...
  {
    name: "web_search",
    description: "Search the web for current information, news, or images. Returns concise, attributed results with titles, URLs, and snippets. Use this when the user asks about recent events, facts that may have changed, or anything not in the model's training data.",
    ...
    types: {
      ...
      timeRange: "enum (optional)",
      includeRawContent: "boolean (optional)",
      includeImages: "boolean (optional)",
    },
    requiredArgs: ["query"],
    optionalArgs: ["provider", "type", "limit", "language", "country", "timeRange", "includeRawContent", "includeImages"],
    validFormats: [
      'web_search {"query": "latest linux kernel release"}',
      'web_search {"query": "Mars rover news", "type": "news", "limit": 3}',
      'web_search {"query": "desklumina github", "provider": "tavily"}',
    ],
    invalidFormats: [
      'web_search latest linux kernel',
      'web_search {"query": ""}',
      'web_search {"provider": "tavily"}',
    ],
    escapingRules: "JSON-standard backslash escaping for quotes in payload.",
    quotingRules: "Arguments MUST be a valid JSON object string. Double quotes for keys and values.",
    output: {
      success: "Concise result list with titles, URLs, snippets, and rank.",
      failure: "Error message naming the provider and failure class.",
      empty: "No results found message.",
      partial: "Partial results with warnings naming failed providers.",
    },
    failure: {
      retriable: ["Timeout", "Network error", "Rate limit (429)", "Provider unavailable (5xx)", "Malformed response"],
      nonRetriable: ["Invalid query", "Authentication failed", "Missing configuration", "Unsupported capability", "Invalid request parameters"],
      retryLimit: 0,
      permissionBehavior: "N/A",
      malformedInputBehavior: "Returns exit code 2 and usage error.",
    },
    formatAnchors: [
      '{"tool":"web_search","args":"{\\"query\\":\\"latest Mars rover news\\"}"}',
